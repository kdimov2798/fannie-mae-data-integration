...
SELECT 
  'TARGET',
  COUNT(*)
FROM `dmi_target.msp_loans_trial`;
...
FROM random_loans r
JOIN `dmi_staging.loan_master_trial` s ON r.loan_identifier = s.loan_identifier
JOIN `dmi_target.msp_loans_trial` t ON CAST(s.loan_identifier AS STRING) = t.loan_id;